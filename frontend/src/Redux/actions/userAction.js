...
export const userLogin = (email, password) => async (dispatch) => {
    try {
        ...
        const user = await axios.post("/api/v1/login", { email, password }, config)
        dispatch({
            type: LOGIN_SUCCESS,
            payload: user.data,
        })
...
export const userRegister = (userData) => async (dispatch) => {
    try {
        ...
        const user = await axios.post(`/api/v1/register`, userData, config);
...
export const resetPassword = (token, passwords) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`/api/v1/password/reset/${token}`,
            { token, password: passwords.password, confirmPassword: passwords.confirmPassword }, config)
        dispatch({
